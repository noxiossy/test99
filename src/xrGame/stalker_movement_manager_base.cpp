...
public:
	inline	sight_manager_enable_guard	(CSightManager& manager, bool const value) :
		m_manager		(manager),
		m_value			(value)
	{
	}
...
void stalker_movement_manager_base::setup_movement_params	(stalker_movement_params& movement_params)
{
	...
	if (movement_params.desired_position()) {
		...
		if (
				(movement_params.m_path_type != MovementManager::ePathTypePatrolPath) &&
...
void stalker_movement_manager_base::force_update	(const bool &force_update)
{
