	...
	if(m_bIsHorizontal)
	{
		if(m_ScrollBox->GetWndRect().left <= m_DecButton->GetWidth()+m_ScrollBoxOffset.x)
			m_ScrollBox->SetWndPos(Fvector2().set(m_ScrollBoxOffset.x+m_DecButton->GetWidth(), m_ScrollBox->GetWndRect().top));
				...
				m_ScrollBox->SetWndPos(Fvector2().set(m_IncButton->GetWndRect().left - m_ScrollBox->GetWidth()-m_ScrollBoxOffset.x, 
	...
	else
	{
		if(m_ScrollBox->GetWndRect().top <= m_DecButton->GetHeight()+m_ScrollBoxOffset.y)
			m_ScrollBox->SetWndPos(Fvector2().set(m_ScrollBox->GetWndRect().left, 
									m_ScrollBoxOffset.y+m_DecButton->GetHeight()));
...
void CUIFixedScrollBar::OnMouseUp(int mouse_btn)
{
	m_mouse_state = 0;
}
