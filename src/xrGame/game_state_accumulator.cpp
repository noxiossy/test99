...
void game_state_accumulator::OnBullet_Fire(u16 sender,
										   u16 sender_weapon_id,
...
void game_state_accumulator::init_player_accum_values(game_PlayerState* new_local_player)
{
	...
	{
		accumulative_values_collection_t::iterator tmp_iter = m_accumulative_values.find(
			static_cast<enum_accumulative_player_values>(i));
...
bool game_state_accumulator::check_hit_params(u32 count,
												...
												float right_dist_arg)
{
	struct hit_fetcher
	{
		bool			operator()	(shared_str const & hitter,
									 shared_str const & victim,
									 hits_store::bullet_hit const & hit)
		{
...
bool game_state_accumulator::check_kill_params(u32 count,
												 ammunition_group::enum_group_id weapon_group_id,
												 KILL_TYPE kill_type,
												 ...
												 u32 time_period)
{
	struct kills_fetcher
	{
									 ...
									 kills_store::kill const & kill)
		{
			...
			if ((m_spec_kill != SKT_NONE) &&
				(kill.m_spec_kill_type != m_spec_kill))
			{
				return false;
			}
...
bool game_state_accumulator::check_accumulative_value(enum_accumulative_player_values param_id,
														float_binary_function*	func,
														float right_arg)
{
	accumulative_values_collection_t::iterator	tmp_iter = m_accumulative_values.find(
...
bool game_state_accumulator::is_enemies	(game_PlayerState const * left_player,
