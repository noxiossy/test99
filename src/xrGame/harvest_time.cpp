...
harvest_time::harvest_time(game_state_accumulator* owner) :
	inherited(owner)
{
	m_harvest_count = 0;
	m_spawn_time	= 0;
...
{
	victim_raw_kill					(shared_str const & killer,
									 shared_str const & victim,
									 u32 const after_time) :
		m_killer(killer),
		m_victim(victim),
...
void harvest_time::OnPlayerKilled(u16 killer_id,
								  u16 target_id, 
								  u16 weapon_id, 
