...
player_state_cherub::player_state_cherub(game_state_accumulator* owner) :
	inherited(owner)
{
	m_kill_count		= 0;
	m_art_take_time		= 0;
...
void player_state_cherub::OnPlayerKilled		(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	struct hit_fetcher
	{
		...
		{
			if ((hitter == m_hitter_name) &&
				(victim == m_victim_name) &&
				(hit.m_hit_time >= m_art_take_time))
			{
				return true;
			}
			return false;
