...
player_state_skewer::player_state_skewer(game_state_accumulator* owner) :
	inherited(owner)
{
	m_shot			= 1;
	m_kills_count	= 0;
...
void player_state_skewer::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	game_PlayerState* tmp_local_player = m_owner->get_local_player();
	if (!tmp_local_player)
		return;
	...
	if (m_owner->is_item_in_group(
		...
		ammunition_group::gid_gauss_rifle))
	{
		if (m_shot)
		{
			m_kills_count	= 1;
