...
double_shot_double_kill::double_shot_double_kill(game_state_accumulator* owner) :
	inherited(owner)
{
	m_shot_count	= 0;
}
...
void double_shot_double_kill::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	game_PlayerState* tmp_local_player = m_owner->get_local_player();
	if (!tmp_local_player)
		return;
	...
	if (!m_owner->is_item_in_group(
		m_owner->get_object_id(weapon_id),
		ammunition_group::gid_double_barred))
	{
		return;
	}
