...
player_state_invincible_fury::player_state_invincible_fury(game_state_accumulator* owner) :
	inherited(owner)
{
	m_last_kills	= 0;
}

u32 const player_state_invincible_fury::get_u32_param()
{
	return m_last_kills;
}

void player_state_invincible_fury::reset_game()
{
	m_last_kills	= 0;
}
...
struct fury_killer
{
	fury_killer						(shared_str const & killer_name,
									 u32 after_time) :
		m_killer_name(killer_name),
...
void player_state_invincible_fury::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	...
	m_last_kills = 0;
	for (buffer_vector<kills_store::kill>::const_iterator i = tmp_buffer.begin(),
		ie = tmp_buffer.end(); i != ie; ++i)
	{
