...
player_state_marksman::player_state_marksman(game_state_accumulator* owner) :
	inherited(owner)
{
	m_spawn_time	= 0;
}

u32 const player_state_marksman::get_u32_param()
{
	return m_sniper_victims.size();
}
...
void player_state_marksman::OnPlayerKilled(u16 killer_id,
								  u16 target_id, 
								  u16 weapon_id, 
								  std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	...
	if (killer_actor && victim_actor)
	{
		...
		{
			m_sniper_victims.insert(
				std::make_pair(victim_actor->cName(), kill_dist));
		}
	}
}
