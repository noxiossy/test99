...
player_death_counter::player_death_counter(game_state_accumulator* owner) :
	inherited(owner)
{
}
...
player_artdeliver_counter::player_artdeliver_counter(game_state_accumulator* owner) :
	inherited(owner)
{
}
...
player_rawkill_counter::player_rawkill_counter(game_state_accumulator* owner) :
	inherited(owner)
{
	m_raw_kills		= 0;
}
...
void player_rawkill_counter::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	game_PlayerState* tmp_local_player = m_owner->get_local_player();
	if (!tmp_local_player)
		return;
...
player_state_move::player_state_move(game_state_accumulator* owner) :
	inherited(owner)
{
}
...
u32 const player_state_move::get_u32_param()
{
	...
	return state.bCrouch | 
		(state.bSprint	<< 1) | 
		(state.bJump	<< 2) | 
		(state.bFall	<< 3);
}

player_state_velocity::player_state_velocity(game_state_accumulator* owner) :
	inherited(owner)
{
}
...
player_state_ang_velocity::player_state_ang_velocity(game_state_accumulator* owner) :
	inherited(owner)
{
}
