...
faster_than_bullets_time::faster_than_bullets_time(game_state_accumulator* owner) :
	inherited(owner)
{
	m_no_demag_time = u32(-1);
}

u32 const faster_than_bullets_time::get_u32_param()
{
	return m_no_demag_time;
}

void faster_than_bullets_time::reset_game()
{
	m_no_demag_time	= u32(-1);
}
...
struct last_hits_fetcher
{
	...
	bool operator () (shared_str const & k_name, 
					  shared_str const & v_name,
					  hits_store::bullet_hit const & hit)
	{
		if ((k_name == m_killer_name) &&
			(v_name == m_victim_name))
		{
...
void faster_than_bullets_time::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
	...
	if ((killer_id != tmp_local_player->GameID) ||
		(kill_type.second != SKT_KNIFEKILL))
	{
		return;
	}
