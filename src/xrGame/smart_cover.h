...
public:
	...
	IC		Fvector				position				(Fvector const &position) const;
			...
			loophole			*best_loophole			(Fvector const &position, float &value, bool const &use_default_behaviour, bool is_smart_cover_entered) const;
	IC		DescriptionPtr const&description			() const;
			void				evaluate_loophole		(Fvector const &position, loophole * &source, loophole * &result, float &value, bool is_smart_cover_entered) const;
