...
achilles_heel_kill::achilles_heel_kill(game_state_accumulator* owner) :
	inherited(owner)
{
	m_achilles_kill_was	= false;
}

u32 const achilles_heel_kill::get_u32_param()
{
	return m_achilles_kill_was ? 1 : 0;
}

void achilles_heel_kill::reset_game()
{
	m_achilles_kill_was	= false;
}
