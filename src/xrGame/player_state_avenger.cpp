...
player_state_avenger::player_state_avenger(game_state_accumulator* owner) :
	inherited(owner)
{
	m_aveng_count			= 0;
};
	...
	struct need_revenge
	{
										 ...
										 kills_store::kill const & kill)
		{
			if ((killer == m_killer_name) &&
				(kill.m_kill_time >= m_killer_spawn_time))
			{
				if (std::find(
						m_my_team_players.begin(),
						m_my_team_players.end(),
						victim) != m_my_team_players.end())
				{
					return true;
				}
			}
...
void player_state_avenger::feel_my_team_players(game_PlayerState const * of_player,
												buffer_vector<shared_str> & dest)
{
	game_PlayerState const * tmp_local_player = m_owner->get_local_player();
