...
bool account_manager::verify_unique_nick	(char const * unick)
{
	...
	{
		bool valid = (unick[i] >= 34) && (unick[i] <= 126) && 
...
bool account_manager::verify_email	(char const * email)
{
	...
	char const * tmp_char = std::find(
		email,
		tmp_endchar,
		'@'
	);
	if ((tmp_char == tmp_endchar)	||
		(tmp_char == email) ||
		(tmp_char + 1 == tmp_endchar) || 
		!isalnum(*(tmp_char + 1)) ||
		!isalnum(*(tmp_char - 1)))
	{
		Msg("! ERROR: bad email");
...
bool account_manager::is_get_account_profiles_active() const
{
	return m_get_account_profiles_qam.is_active();
}
...
void account_manager::stop_fetching_account_profiles()
{
	m_get_account_profiles_qam.stop();
}

void account_manager::get_account_profiles_raw(get_account_params_t const & args,
											 account_profiles_cb profiles_cb)
{
...
void account_manager::search_for_email(char const * email,
									 found_email_cb found_cb)
{
	if (!found_cb)
	{
...
bool account_manager::is_email_searching_active	() const
{
	return m_search_for_email_qam.is_active();
}
...
void account_manager::stop_searching_email()
{
	m_search_for_email_qam.stop();
}

void account_manager::suggest_unique_nicks(char const * unick,
...
void account_manager::stop_suggest_unique_nicks()
{
	m_suggest_uniqie_nicks_qam.stop();
}

void account_manager::suggest_unique_nicks_raw(suggest_uniqie_nicks_params_t const & unick,
											   suggest_nicks_cb sncb)
{
...
bool account_manager::is_suggest_unique_nicks_active() const
{
	return m_suggest_uniqie_nicks_qam.is_active();
}
...
void __cdecl	account_manager::new_user_cb(GPConnection * connection,
											 void * arg,
											 void * param)
{
