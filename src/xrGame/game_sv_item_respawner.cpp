...
void item_respawn_manager::clear_respawn_sections()
{
	...
	for (respawn_section_iter	temp_iter = m_respawn_sections_cache.begin();
		temp_iter != temp_ie; ++temp_iter)
	{
...
void item_respawn_manager::update(u32 current_time)
{
	...
	for (respawn_iter i = m_respawns.begin(); i != ie; ++i)
	{
		if (i->last_spawn_time && 
			((i->last_spawn_time + i->respawn_time) <= current_time))
		{
