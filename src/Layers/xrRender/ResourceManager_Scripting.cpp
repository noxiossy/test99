...
static const char* get_lua_traceback(lua_State *L)
{
	...
	xr_strcpy(buffer, 32767, traceback);
	__try
	{
...
bool print_output(const char* caScriptFileName, int errorCode)
{
	...
	if (!lua_isstring(LSVM, -1)) //ÍÅ ÓÄÀËßÒÜ! Èíà÷å áóäóò âûëeòû áåç ëîãà!
	{
		Msg("*********************************************************************************");
...
{
	//KRodin: Ïåðåäåëàë, ò.ê. â îðèãèíàëå òóò ïðîèñõîäèëî íå÷òî, íà ìîé âçãëÿä, ñòðàííîå.
	...
	//Log(strBuf.get());
	int l_iErrorCode = luaL_loadbuffer(LSVM, strBuf.get(), buf_len /*+ 1 Íóëü-òåðìèíàòîð íà êîíöå ìåøàåò ïîõîäó*/, caScriptName);
	if (l_iErrorCode)
	{
		print_output(caScriptName, l_iErrorCode);
		R_ASSERT(false); //ÍÅ ÇÀÊÎÌÌÅÍÒÈÐÎÂÀÒÜ!
		return false;
	}
	return true;
}
...
bool do_file(const char* caScriptName, const char* caNameSpaceName)
{
	...
	if (!l_tpFileReader) { //çàìåíèòü íà àññåðò?
		Msg("!![CResourceManager::do_file] Cannot open file [%s]", caScriptName);
		return false;
	}
	strconcat(sizeof(l_caLuaFileName), l_caLuaFileName, "@", caScriptName); //KRodin: ïðèâîäèò ïóòü ê âèäó @f:\games\s.t.a.l.k.e.r\gamedata\scripts\class_registrator.script
	//
	//KRodin: èñïðàâëåíî. Òåïåðü ñîäåðæèìîå ñêðèïòà ñðàçó ÷èòàåòñÿ íîðìàëüíî, áåç ìóñîðà íà êîíöå.
	...
	int	l_iErrorCode = lua_pcall(LSVM, 0, 0, 0); //KRodin: áåç ýòîãî ñêðèïòû íå ðàáîòàþò!
	if (l_iErrorCode)
	{
		print_output(caScriptName, l_iErrorCode);
		R_ASSERT(false); //ÍÅ ÇÀÊÎÌÌÅÍÒÈÐÎÂÀÒÜ!
