...
{
	{
		...
		static_assert(a::value, "Client class must be derived from the CLIENT BASE CLASS");
	}
	{
		typedef object_type_traits::is_base_and_derived<SERVER_BASE_CLASS,_server_type> a;
		static_assert(a::value, "Server class must be derived from the SERVER BASE CLASS");
	}
...
{
	{
		...
		static_assert(a::value || b::value, "Class must be derived from the CLIENT BASE CLASS or SERVER BASE CLASS");
	}
	add					(
		xr_new<
			CObjectItemSingle<
