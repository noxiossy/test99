...
//======================================
//используется экземплярами объектов для регистрации себя 
//чтобы можно было отдавать интерфейсы, которые реализует этот экземпляр 
//(экземпляры одного и того же различаются по ExData)
DECLARE_INTERFACE_(IInstanceInterfaceProvider, IUnknown)
{
 ...
 //каждый экземпляр объекта регистрирует этот метод в registry в конструкторе,
 //чтобы по запросу отдавать указатели на интерфейсы, которые он реализует
 //экземпляры распознаются по Exdata
 //НЕОБХОДИМО УДАЛАТЬ ИНТЕРФЕЙС ИЗ РЕГИСТРИ В ДЕСТРУКТОРЕ ЭКЗЕМПЛЯРА, ИНАЧЕ ИСПОРТИТСЯ ВСЯ СИСТЕМА !!!
 //рекомендуется наследовать от класа TInstanceInterfaceProvider
...
//===================================================
//применяется при описании класа, реализующего интерфейс, создаваемого спомощью factory method
