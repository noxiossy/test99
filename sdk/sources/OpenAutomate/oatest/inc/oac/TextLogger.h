...
#ifndef _oac_TextLogger_h
#define _oac_TextLogger_h

#include <oac/Logger.h>
#include <stdio.h>

//
// Converts oacLog::Entries into plain text.  This class shouldn't be used directly.
// Instead, loggers that store the logs as individual lines of text should derive
// from this clas, and implement EmitLine().
//
class oacTextLogger : public oacLogger
{
public:

  oacTextLogger(int max_columns = 120);
  ~oacTextLogger();

  void SetMaxColumns(int columns);

  virtual void Emit(const oacLog::Entry &entry, int group_level);
