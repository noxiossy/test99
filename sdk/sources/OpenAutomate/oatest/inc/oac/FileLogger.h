...
#ifndef _oac_FileLogger_h
#define _oac_FileLogger_h

#include <oac/TextLogger.h>
#include <stdio.h>

class oacFileLogger : public oacTextLogger
{
public:

  oacFileLogger(int max_columns = 120);
  ~oacFileLogger();

  bool SetFile(FILE *file);
  bool SetFile(const char *filename, bool append = false);

  virtual void EmitLine(const char *line);
