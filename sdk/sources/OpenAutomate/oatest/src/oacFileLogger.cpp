...
oacFileLogger::oacFileLogger(int max_columns) : 
  oacTextLogger(max_columns)
{
  pFile = NULL;
  pEnabled = true;
}

oacFileLogger::~oacFileLogger()
{
  if(pFile)
    fclose(pFile);
}

bool oacFileLogger::SetFile(FILE *file)
{
  pFile = file;
  return(true);
}

bool oacFileLogger::SetFile(const char *filename, bool append)
{
  assert(filename != NULL);
  const char *OpenMode = append ? "ab" : "wb";
...
void oacFileLogger::EmitLine(const char *line)
{
  if(!pEnabled)
    return;
...
void oacFileLogger::SetEnabled(bool flag)
{
  pEnabled = flag;
}
