...
OAC_SIMPLE_TEST_HEAD(benchmarks, 
                     "Basic sanity tests for OA_CMD_GET_BENCHMARKS",
                     2, 
                     true) 
  ...
  OAC_TEST_MSG(Benchmarks.size() > 0, 
    "There must be at least one benchmark defined.");

  map<string, int> Counts;
  vector<string>::const_iterator VIter;
  for(VIter=Benchmarks.begin(); VIter != Benchmarks.end(); ++VIter)
    Counts[*VIter] = 1;

  OAC_TEST_MSG(Counts.size() == Benchmarks.size(),
    "Some benchmark names were added more than once.")
